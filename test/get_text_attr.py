...
class GetTextOrAttribute(unittest.TestCase):
	...
	def test_GetTextOrAttribute(self):
		...
		seePhotosButtonElement = WebDriverWait(driver, 10).until(lambda driver: driver.find_element_by_xpath(seePhotosButtonLocator))
		text = seePhotosButtonElement.text
		print text

		classAttr = seePhotosButtonElement.get_attribute("class")
		print classAttr
